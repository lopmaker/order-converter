        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Review the generated PO before sending to vendor.
                    </DialogDescription>
                </DialogHeader>

                <div ref={printRef} className="border p-8 bg-white text-black shadow-sm my-4 text-sm print:border-0 print:shadow-none">
                    {/* PO Header */}
                    <div className="flex justify-between mb-8">
                        ...
                        <div className="text-right">
                            ...
                            {data.soReference && <p className="text-gray-500 text-xs mt-1">Ref: {data.soReference}</p>}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-6 mb-6 text-xs">
                        {data.shipVia && (
                            <div><span className="font-bold">Ship Via:</span> {data.shipVia}</div>
                        )}
                        {data.shipmentTerms && (
                            <div><span className="font-bold">Terms:</span> {data.shipmentTerms}</div>
                        )}
                        {data.paymentTerms && (
                            <div><span className="font-bold">Payment:</span> {data.paymentTerms}</div>
                        )}
                        {data.agent && (
                            <div><span className="font-bold">Agent:</span> {data.agent}</div>
                        )}
                    </div>
                    ...
                    <table className="w-full mb-8">
                        ...
                        <tbody className="divide-y">
                            {data.items.length > 0 ? (
                                data.items.map((item, idx) => (
                                    <tr key={idx} style={{ breakInside: 'avoid' }}>
                                        <td className="py-2">
                                            ...
                                            <div className="text-gray-600 truncate max-w-sm text-xs">{item.description}</div>
                                ...
                                <tr>
                                    <td colSpan={4} className="py-4 text-center text-gray-400 italic">No items found</td>
                                </tr>
                            )}
                        </tbody>
                        <tfoot>
                            <tr className="border-t-2 border-black">
                                ...
                                <td className="py-2 text-right font-bold">${total.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                            </tr>
                        </tfoot>
                    </table>
                    ...
                    <div className="text-xs text-gray-500 mt-12 pt-4 border-t">
                        <p><strong>Terms & Conditions:</strong> {data.paymentTerms || 'Payment due within 30 days'}. Please include PO number on all invoices.</p>
                    </div>
                </div>
